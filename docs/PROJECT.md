...
### 3.8 Identity (demo mode)

**Problem:** Hackathon and demo deployments need a stable owner id for history without running a full auth stack.

**Solution:** The verify API assigns a **demo user** id when no authenticated session exists. The UI does not ship a third-party auth provider.

**Where:** `src/app/api/verify/route.ts`, `src/components/layout/Header.tsx`.
...
| Path                                       | Responsibility                                                                         |
| ------------------------------------------ | -------------------------------------------------------------------------------------- |
| `src/app/layout.tsx`                       | Root layout, global CSS import, **`<Header />`**, **`<Footer />`**                     |
| `src/app/(marketing)/page.tsx`             | Landing / marketing home (`/`)                                                         |
...
| `userId`                                                   | Owner id (demo user when no session)                                                       |
...
_This handbook describes the **intent** and **current implementation** of the Drygate repository. When code and docs diverge, treat the source files listed in section 6 as authoritative and update this document in the same change._
